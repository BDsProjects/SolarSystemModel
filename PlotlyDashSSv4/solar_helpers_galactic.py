...
solar_regions = {
    ...
    'Kuiper Belt': {'inner_radius': 30, 'outer_radius': 50, 'color': 'lightblue', 'opacity': 0.15},
    'Oort Cloud': {'inner_radius': 2000, 'outer_radius': 100000, 'color': 'rgba(200,200,255,0.3)', 'opacity': 0.05},
    'Termination Shock': {'inner_radius': 80, 'outer_radius': 100, 'tailward_offset': 80, 'north_distance': 20, 'port_side': 15, 'color': 'rgba(255,100,100,0.3)', 'opacity': 0.3},
    'Bow Shock': {'standoff_distance': 130, 'base_radius': 180, 'length': 1500, 'width_factor': 0.8, 'height_factor': 0.5, 'color': 'rgba(255,150,100,0.25)', 'opacity': 0.2},
    'Heliotail': {'base_radius': 100, 'length': 800, 'width_factor': 0.6, 'height_factor': 0.5, 'color': 'rgba(200,100,100,0.2)', 'opacity': 0.15}
}

# Moon parameters (kept for reference but not used in current implementation)
moon_colors = {
    'Moon': '#8c8680', 'Phobos': '#e6c89c', 'Deimos': '#4f71be', 'Io': '#d1603d',
    'Europa': '#e0ae6f', 'Ganymede': '#c5ab6e', 'Callisto': '#9fc4e7', 'Titan': '#4f71be',
    'Ariel': '#8c8680', 'Umbriel': '#ab9c8a', 'Titania': '#d9d9d9', 'Oberon': '#d9d9d9',
...
class PlanetaryPositionDB:
    """Handle database queries for planetary positions."""
    ...
    def _load_cache(self):
        ...
        query = """
        SELECT planet_name, date, x_ecliptic, y_ecliptic, z_ecliptic, 
               x_galactic, y_galactic, z_galactic
        FROM planetary_positions
        ORDER BY planet_name, date
        """
        self.df = pd.read_sql_query(query, self.conn)
        
        # Don't convert to pandas datetime due to 2262 limitation
        # Instead, convert dates to Python datetime objects for interpolation
        self.df['date_obj'] = pd.to_datetime(self.df['date'], errors='coerce')
        
        # For dates beyond pandas range, parse manually
        for idx, row in self.df.iterrows():
            if pd.isna(row['date_obj']):
                date_str = row['date']
                year, month, day = map(int, date_str.split('-'))
                self.df.loc[idx, 'date_obj'] = datetime(year, month, day)
        
        # Create interpolation functions for each planet
        self.interpolators = {}
        for planet in self.df['planet_name'].unique():
            planet_data = self.df[self.df['planet_name'] == planet].copy()
            
            # Convert dates to numeric values for interpolation (days since 1900-01-01)
            reference_date = datetime(1900, 1, 1)
            numeric_dates = [(d - reference_date).total_seconds() / 86400.0 
                           for d in planet_data['date_obj']]
            
            self.interpolators[planet] = {
                'x_ecliptic': interp1d(numeric_dates, planet_data['x_ecliptic'], 
                                      kind='cubic', fill_value='extrapolate'),
                'y_ecliptic': interp1d(numeric_dates, planet_data['y_ecliptic'], 
                                      kind='cubic', fill_value='extrapolate'),
                'z_ecliptic': interp1d(numeric_dates, planet_data['z_ecliptic'], 
                                      kind='cubic', fill_value='extrapolate'),
                'x_galactic': interp1d(numeric_dates, planet_data['x_galactic'], 
                                      kind='cubic', fill_value='extrapolate'),
                'y_galactic': interp1d(numeric_dates, planet_data['y_galactic'], 
                                      kind='cubic', fill_value='extrapolate'),
                'z_galactic': interp1d(numeric_dates, planet_data['z_galactic'], 
                                      kind='cubic', fill_value='extrapolate')
            }
    
    def get_position(self, planet_name, date, coordinate_system='ecliptic'):
        """
        Get interpolated position for a planet at any date.
        ...
        """
        if planet_name not in self.interpolators:
            raise ValueError(f"Unknown planet: {planet_name}")
        
        # Convert date to numeric value (days since 1900-01-01)
        reference_date = datetime(1900, 1, 1)
        numeric_date = (date - reference_date).total_seconds() / 86400.0
        
        if coordinate_system == 'ecliptic':
            x = float(self.interpolators[planet_name]['x_ecliptic'](numeric_date))
            y = float(self.interpolators[planet_name]['y_ecliptic'](numeric_date))
            z = float(self.interpolators[planet_name]['z_ecliptic'](numeric_date))
        else:
            x = float(self.interpolators[planet_name]['x_galactic'](numeric_date))
            y = float(self.interpolators[planet_name]['y_galactic'](numeric_date))
            z = float(self.interpolators[planet_name]['z_galactic'](numeric_date))
        
        return x, y, z
    
    def get_orbit_path(self, planet_name, start_date, end_date, num_points=100, 
                      coordinate_system='ecliptic'):
        """
        Get orbit path for a planet between two dates.
        ...
        --------
        tuple
            (x_array, y_array, z_array) arrays of positions
        """
        dates = pd.date_range(start_date, end_date, periods=num_points)
        x_vals, y_vals, z_vals = [], [], []
        
        for date in dates:
            x, y, z = self.get_position(planet_name, date, coordinate_system)
            x_vals.append(x)
            y_vals.append(y)
            z_vals.append(z)
    ...
    def get_available_planets(self):
        """Get list of available planets in the database."""
...
db = PlanetaryPositionDB()

# Galactic Coordinate System class (unchanged)
class GalacticCoordinates:
    """
    Handles transformations between Solar System coordinates (ecliptic) and Galactic coordinates.
    """
    ...
    def __init__(self):
        # J2000 epoch values
        self.pole_ra = np.radians(192.859508)
        self.pole_dec = np.radians(27.128336)
        self.l0 = np.radians(122.932)
...
# Simplified function to get planetary position from database
def calculate_orbital_position(planet_name, time, use_time=True):
    """
    Get the position of a planet at a given time from the database.
    ...
    planet_name : str
        Name of the planet
    time : datetime
        Date/time for the position
    use_time : bool
        If False, returns a static position (for compatibility)
    ...
    """
    if use_time:
        return db.get_position(planet_name, time, 'ecliptic')
    else:
        # For static positions, use J2000.0 epoch
        j2000 = datetime(2000, 1, 1, 12, 0, 0)
        return db.get_position(planet_name, j2000, 'ecliptic')

# Helper: create a spherical shell (unchanged)
def create_shell(inner_radius, outer_radius=None, color='gray', opacity=0.2, points=40, use_galactic=False):
    if outer_radius is None:  # Simple sphere
        u, v = np.mgrid[0:2*np.pi:points*1j, 0:np.pi:points//2*1j]
        x = inner_radius * np.cos(u) * np.sin(v)
...
# Keep all the visualization helper functions unchanged
def create_bow_shock_ellipsoid(standoff_distance=130, base_radius=180, length=1500, 
                             width_factor=0.85, height_factor=0.85, 
                             color='rgba(255,150,100,0.25)', opacity=0.2, 
                             points=250, use_galactic=False):
...
def create_heliotail(base_radius=120, length=1000, width_factor=0.6, height_factor=0.5,
    ...
    """Create an elongated tail structure extending from the termination shock."""
    u = np.linspace(0, 2*np.pi, points)
    v = np.linspace(0, 1, points//2)
    u_grid, v_grid = np.meshgrid(u, v)
    
    radius = base_radius * (1 - v_grid*0.8)
...
def create_ellipsoid(inner_edge=0, length=950, width_factor=0.75, height_factor=0.5, 
                    color='rgba(255,100,100,0.3)', opacity=0.2, name="Extended Region", 
                    points=40, use_galactic=False):
    """Create a see-through ellipsoid with left edge at a specified distance."""
    center_x = inner_edge + length/2
    a = length/2
    b = length * width_factor/2
    c = length * height_factor/2
    
    u, v = np.mgrid[0:2*np.pi:points*1j, 0:np.pi:points//2*1j]
    
    x = center_x + a * np.cos(u) * np.sin(v)
    y = b * np.sin(u) * np.sin(v)
    z = c * np.cos(v)
...
def create_disk(inner_radius, outer_radius, color='gray', opacity=0.2, points=1000, use_galactic=False):
    """Create a disk (for asteroid belt)."""
    r = np.sqrt(np.random.uniform(inner_radius**2, outer_radius**2, points))
    theta = np.random.uniform(0, 2*np.pi, points)
...
# Enhanced make_traces function using database
def make_traces_time(planet_list, current_time=None, show_trails=True, trail_days=365, 
                    include_regions=True, use_planet_spheres=True, 
                    include_ellipsoid=False, ellipsoid_params=None, use_galactic=False):
    """Create traces for visualization using database positions."""
    traces = []
    ...
    # Planets with time-based positions from database
    for planet_name in planet_list:
        # Get current position from database
        coord_system = 'galactic' if use_galactic else 'ecliptic'
        x, y, z = db.get_position(planet_name, current_time, coord_system)
        
        # Create planet representation
        if use_planet_spheres:
            planet_radius = planet_relative_sizes.get(planet_name, 0.1) * size_scale_factor
            planet_trace = create_planet_sphere(
                x, y, z, planet_radius, 
                planet_colors.get(planet_name, '#999999'), planet_name,
                use_galactic=False  # Already in correct coordinate system
            )
            traces.append(planet_trace)
        else:
            traces.append(go.Scatter3d(
                x=[x], y=[y], z=[z],
                mode='markers', 
                marker=dict(size=8, color=planet_colors.get(planet_name, '#999999')), 
                name=planet_name
            ))
        
        # Add orbital trails if requested
        if show_trails:
            start_date = current_time - timedelta(days=trail_days/2)
            end_date = current_time + timedelta(days=trail_days/2)
            
            # Get orbit path from database
            trail_x, trail_y, trail_z = db.get_orbit_path(
                planet_name, start_date, end_date, 
                num_points=50, coordinate_system=coord_system
            )
            
            # Split trail into past and future
            current_idx = len(trail_x) // 2
            
            # Past trail (dimmer)
            traces.append(go.Scatter3d(
                x=trail_x[:current_idx], 
                y=trail_y[:current_idx], 
                z=trail_z[:current_idx],
                mode='lines',
                line=dict(color=planet_colors.get(planet_name, '#999999'), width=2),
                opacity=0.3,
                showlegend=False,
                hoverinfo='skip'
            ))
            
            # Future trail (brighter)
            traces.append(go.Scatter3d(
                x=trail_x[current_idx:], 
                y=trail_y[current_idx:], 
                z=trail_z[current_idx:],
...
def build_fig_time(subset, rng, title, current_time=None, show_trails=True, 
                  ...
                  use_planet_spheres=True, include_ellipsoid=False, 
                  ellipsoid_params=None, use_galactic=False):
    """Build complete figure with database data."""
    traces = make_traces_time(
        subset, current_time, show_trails, trail_days,
        include_regions, use_planet_spheres, 
        include_ellipsoid, ellipsoid_params, use_galactic
...
def create_time_animation(planet_list, start_date, end_date, frames=30, rng=10,
                        include_regions=True, use_galactic=False, use_planet_spheres=True):
    """Create an animated visualization using database data."""
    # Create time steps
    time_steps = []
    total_days = (end_date - start_date).days
    for i in range(frames):
    ...
    for i, time in enumerate(time_steps):
        ...
        # Regions (static)
        if include_regions:
            frame_data.extend(fig.data[1:3])
        
        # Update planet positions
        for planet_name in planet_list:
            x, y, z = db.get_position(planet_name, time, coord_system)
            ...
            if use_planet_spheres:
                ...
                frame_data.append(go.Surface(
                    x=sphere_x, y=sphere_y, z=sphere_z,
                    colorscale=[[0, planet_colors.get(planet_name, '#999999')], 
                              [1, planet_colors.get(planet_name, '#999999')]],
                    showscale=False,
                    name=planet_name
                ))
            else:
...
# Demo functions
def demo_time_based_positions():
    """Demo showing planets at different times using database."""
    current_date = datetime.now()
...
def demo_planetary_alignment(date=None):
    """Show all planets at a specific date from database."""
    if date is None:
        date = datetime(1900, 1, 1)
    
    all_planets = db.get_available_planets()[:8]  # Main planets
    
    fig = build_fig_time(
        all_planets, 35, f"Planetary Positions", 
        current_time=date, 
        show_trails=False
    )
...
def demo_animation():
    """Create an animation using database data."""
...
def find_planetary_conjunctions(planet1, planet2, start_date, end_date, threshold=5.0):
    """Find planetary conjunctions using database positions."""
    conjunctions = []
    
    current_date = start_date
    while current_date <= end_date:
        # Get positions from database
        x1, y1, z1 = db.get_position(planet1, current_date, 'ecliptic')
        x2, y2, z2 = db.get_position(planet2, current_date, 'ecliptic')
...
def get_planet_distances_from_earth(date):
    """Calculate distances from Earth using database positions."""
    earth_x, earth_y, earth_z = db.get_position('Earth', date, 'ecliptic')
    
    distances = {}
    for planet in db.get_available_planets():
        if planet == 'Earth':
            continue
            
        x, y, z = db.get_position(planet, date, 'ecliptic')
        distance = np.sqrt((x - earth_x)**2 + (y - earth_y)**2 + (z - earth_z)**2)
        distances[planet] = distance
    
    return distances

# Example of using the database-based visualization
if __name__ == "__main__":
    # Demo: Show current planetary positions
    fig = demo_planetary_alignment(datetime(2260, 7, 15))
    fig.show()
    
    # Demo: Create animation for 2024
    fig_anim = demo_animation()
    fig_anim.show()
    
    # Demo: Find conjunctions
    conjunctions = find_planetary_conjunctions(
        'Venus', 'Jupiter', 
        datetime(1900, 1, 1), 
        datetime(2260, 12, 31),
        threshold=10.0
    )
    print(f"Conjunctions in 2024: {len(conjunctions)}")
    for date, sep in conjunctions:
        print(f"  {date.strftime('%Y-%m-%d')}: {sep:.2f}°")